...
public class SimpleConnectionPool {
    ...
    private static SimpleConnectionPool pool = null;

    private SimpleConnectionPool(String url, String user, String password, int maxSize) { // maxSize := MaximumPoolSize
        this.url = url;
        this.user = user;
    ...
    public synchronized Connection getConnection(long timeout) throws SQLException, InterruptedException {
        long startTime = System.currentTimeMillis(); // 메소드 실행 시간을 기록 (이 코드는 반복문 밖에 있음을 명시)
        ...
        System.out.println("스레드 획득 현재 개수 : " + getAvailableConnectionsCount());
